...
- 可自动识别播放器词库，也可手动指定网易云、QQ、酷狗、酷我、汽水或咪咕；手动词库无结果时可选择是否回退到播放器同源词库。
...
设置页的“当前歌曲校正”会单独记住这一首歌的偏移，并叠加到全局和播放器校正；导入的 `.lrc` 内 `[offset:]` 也会生效。
...
- 本地歌词支持 `.lrc`、`.krc`、UTF-8、GB18030 和 UTF-16，单文件上限 10 MB；已授权的目录用于读取匹配歌词。
